...
def save_image(x, ncol, filename, cpu=False):
    x = denormalize(x, cpu=cpu)
    if cpu:
        vutils.save_image(torch.from_numpy(x), filename, nrow=ncol, padding=0)
    else:
...
def translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, filename):
    ...
    save_image(x_concat, N, filename, cpu=True)
...
def translate_using_reference(nets, args, x_src, x_ref, y_ref, filename):
    ...
    x_concat = [x_src_with_wb.cpu()]
    for i, s_ref in enumerate(s_ref_list):
        x_fake = nets.generator(x_src, s_ref, masks=masks)
        x_fake_with_ref = torch.cat([x_ref[i:i+1], x_fake], dim=0)
        x_concat += [x_fake_with_ref.cpu()]

    # x_concat = torch.cat(x_concat, dim=0)
    x_concat = np.concatenate(x_concat, axis=0)
    save_image(x_concat, N+1, filename, cpu=True)
    del x_concat
